...
## output
os.makedirs(('./output/{}/NB/GaussianNB/'.format(task)), exist_ok=True)
file_metrics = './output/{}/NB/GaussianNB/metrics.txt'.format(task)
...
for cv in range(5):
    ...
    ## metrics
    path_train = './output/{}/NB/GaussianNB/cv{}-train.csv'.format(task, cv)
    path_test = './output/{}/NB/GaussianNB/cv{}-test.csv'.format(task, cv)
